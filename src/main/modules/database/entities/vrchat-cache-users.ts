...
export class UserEntity implements UserSummary {
  @PrimaryColumn({ name: 'user_id', type: 'varchar', length: 63 })
  declare userId: string

  @Index('IDX_vrchat_cache_users_user_name')
  @Column({ name: 'display_name', type: 'text' })
  declare displayName: string

  @Column({ name: 'profile_icon_file_id', type: 'text' })
  declare profileIconFileId: string

  @Column({ name: 'profile_icon_file_version', type: 'integer' })
  declare profileIconFileVersion: number

  @Column({ name: 'trust_rank', type: 'varchar', length: 31 })
  declare trustRank: UserTrustRank

  @Column({ name: 'languages', type: 'text', ...propertyTransformer })
  declare languages: UserLanguage[]

  @Column({ name: 'is_supporter', type: 'boolean' })
